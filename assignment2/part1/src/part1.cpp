...
void drawCube(Context &ctx)
{
	...
	float a = (float)(glm::sin(elapsed_time) + 1) * 3.14;
	float b = (float)(glm::cos(elapsed_time) + 1) * 3.14;

	glm::vec3 axis = glm::vec3(a, b, 0);
    // Define the model, view, and projection matrices here
    glm::mat4 model = glm::rotate(glm::mat4(1.0f), a, axis);
	//glm::mat4(1.0f);
